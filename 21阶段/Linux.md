...
* Linux系统构成：
  ...
  * shell命令解释器：它使得用户能够与操作系统进行交互，负责接收用户命令，然后调用操作系统功能。
...
* 部分命令细节说明
  * ls ：  -l 展示详细信息，-a展示隐藏文件（Linux下 . 开头的为隐藏文件）。**【开头是-就是文件夹，开头是d就是文件】**
  * cd： 参数为绝对路径或者相对路径，直接cd表示回到主目录。
  * touch:  可以同时跟多个参数表示创建多个文件。
  * mkdir: -p选项可以创建层目录
  * cp：如果拷贝的是一个目录需要使用 -r ，同时这个命令有**另存为**的作用
  * mv:  即使移动目录页不需要选项，有**重命名**的作用。
...
| 序号 | 命令                  | 作用                                                 |
| :--- | :-------------------  | :--------------------------------------------------- |
| 01   | sudo | 放在一个命令前，表示使用管理员权限执行【根目录下才需要】 |
| 02   | chmod  | 修改文件权限【写权限】    |
...
* 部分命令细节说明
  ...
  * 第一组是创建者权限，第二组是小组内其他成员的权限，第三组是小组外的其他人的权限
  
  ```bash
  直接修改文件|目录的 读|写|执行 权限，但是不能精确到 拥有者|组|其他
  chmod  a【all】u【user】g【group】o【others】   +/-   rwx 文件名/目录名【可执行的名字变绿】
...
* 部分命令细节说明
  ...
  * date > 文件名     文件存在就**清空**原本内容并写入时间，不存在就自动创建【将原本该输出到终端中的时间数据写入到文件中】
  * date>>文件名     文件存在**不会清空**原本内容并写入时间，不存在就自动创建【将原本该输出到终端中的时间数据写入到文件中】
...
* 部分命令细节说明
  ...
  * ln : 一般使用  -s 选项 创建**软链接**【移动位置会报错】，相当于**快捷方式**，【如果跨目录创建软连接要使用**绝对路径**】。
  
    ```shell
    ln -s  hello.py【文件名】  hello
    ```
...
| 命令 |  英文  | 功能                       |  常用  |
| :--: | :----: | -------------------------- | :----: |
|  i   | insert | 在**当前字符前**插入文本   |  常用  |
|  I   | insert | 在**行首**插入文本         | 较常用 |
|  a   | append | 在**当前字符后**添加文本   |        |
...
|    命令      | 功能                              |
| :---------: | --------------------------------- |
|      x       | 删除光标所在字符，或者选中文字【直接退出文件】 |
|     c      | 和移动命令连用,删除光标所在位置到指定位置内容【删除后直接进入到插入模式】 |
...
* 安装启动
  ...
  - 在远程计算机上安装ssh服务 ： sudo apt install openssh-server
...
| 序号 | 命令          | 作用           |
| :--- | :----------- | :------------- |
| 01   | ssh 用户名@ip             【ifconfig】 | 登录远程主机 |
| 02   | scp 用户名@ip:文件名或路径 用户名@ip:文件名或路径【文件夹用-r】 | 远程复制文件   |
...
1. ssh登录
   ...
   ```shell
   ssh  levi@192.168.100.5    # 登录【ifconfig】
   exit                      # 退出
   ```
...
2. scp拷贝
   ...
   scp  levi@192.168.100.5:/home/tarena/demo.py  .【点表示的是当前文件夹】
                                                 ^    
...
* ssh秘钥
  ...
  * 使用方法
    ...
    ```
    1. 在个人计算机中生产秘钥对 ： ssh-keygen  执行以后会在主目录下生成一个.ssh文件夹,其中包含私钥文件id_rsa和公钥文件【给服务器】id_rsa.pub。
    2.cat id-rsa.pub【获得公钥内容】
    3. 在服务器主机上创建文件~/.ssh/authorized_keys，将信任的计算机的id_rsa.pub文件内容追加【echo 公钥内容 >> authorized_keys】到服务器authorized_keys文件中，并修改其权限为chomd 777
```
